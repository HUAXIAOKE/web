const func = (root: HTMLElement, initTheme: string, changeTheme: (theme: string) => void): void => {
	const $ = (s: string): HTMLElement | NodeListOf<HTMLElement> => {
		const dom = root.querySelectorAll<HTMLElement>(s);
	...
	const toggleThemeBasedOnSystem = (): void => {
		if (window.matchMedia('(prefers-color-scheme: dark)').matches) {
			if (!isMoved) {
				components.onclick!(null as any);
			}
		} else {
			if (isMoved) {
				components.onclick!(null as any);
	...
	const moveElementRandomly = (element: HTMLElement): void => {
	...
	const cloudSons = root.querySelectorAll<HTMLElement>('.cloud-son');
	setInterval(() => {
...
class ThemeButton extends HTMLElement {
	...
	connectedCallback(): void {
		...
		const changeTheme = (detail: string): void => {
