...
/** 单条件 或 多条件组(OR of AND)：如 [[{questionId,value}],[{q,v},{q,v}]] 表示 (A) 或 (B且C) */
export type ShowWhenCondition = ShowWhen | ShowWhen[][];
...
function getFormRefValue(form: HTMLFormElement, questionId: string): string {
	const el = form.querySelector(`[name="${questionId}"]`);
	if (el instanceof HTMLInputElement || el instanceof HTMLTextAreaElement) return el.value;
	return '';
}

function matchShowWhen(form: HTMLFormElement, showWhen: ShowWhenCondition): boolean {
	if (!Array.isArray(showWhen)) {
		const single = showWhen as ShowWhen;
		const values = Array.isArray(single.value) ? single.value : [single.value];
		return values.includes(getFormRefValue(form, single.questionId));
	}
	const groups = showWhen as ShowWhen[][];
	return groups.some((andGroup) =>
		andGroup.every((c: ShowWhen) => {
			const vals = Array.isArray(c.value) ? c.value : [c.value];
			return vals.includes(getFormRefValue(form, c.questionId));
		})
	);
}

function collectRefIds(showWhen: ShowWhenCondition): string[] {
	if (!Array.isArray(showWhen)) return [(showWhen as ShowWhen).questionId];
	const groups = showWhen as ShowWhen[][];
	const ids: string[] = [];
	groups.forEach((andGroup) => andGroup.forEach((c) => ids.push(c.questionId)));
	return [...new Set(ids)];
}

/** 依赖「是否为在校生」或「是否线下面试」的题目，切换在校/非在校时清空，避免跨分支保留选项 */
const STUDENT_DEPENDENT_IDS = [
	'college',
	'grade',
	'offline_interview',
	'interview_time_offline',
	'online_interview',
	'interview_time_online',
];
...
function initLogicConditions(form: HTMLFormElement, config: FormConfig): void {
	...
	config.questions.forEach((q, i) => {
		...
		collectRefIds(showWhen).forEach((id) => allRefIds.add(id));
		const show = (): void => {
		...
		const updateVisibility = (): void => {
			if (matchShowWhen(form, showWhen)) show();
			else hide();
		};
		updates.push(updateVisibility);
		updateVisibility();
	});

	function refreshAllLogic(): void {
		updates.forEach((fn) => fn());
	}

	function clearStudentDependentFields(): void {
		STUDENT_DEPENDENT_IDS.forEach((id) => {
			const field = form.querySelector<HTMLElement>(`.joinus-field[data-question-id="${id}"]`);
			const q = config.questions.find((x) => x.id === id);
			if (field && q) clearField(field, q);
		});
	}

	function onRefChange(refId: string): void {
		if (refId === 'student') clearStudentDependentFields();
		refreshAllLogic();
	}
	...
	allRefIds.forEach((refId) => {
		...
		if (refEl) {
			refEl.addEventListener('change', () => onRefChange(refId));
			refEl.addEventListener('input', () => onRefChange(refId));
		}
	});
}
...
export function renderForm(container: HTMLElement, config: FormConfig): void {
	...
	const submitLabel = config.submit?.label ?? '立即提交';
	btn.innerHTML = `<span>${submitLabel}</span><i class="ri-arrow-right-line"></i>`;
	...
	function setSubmitLoading(loading: boolean): void {
		...
		if (loading) {
			...
			submitBtn.classList.add('joinus-btn-loading');
			submitBtn.innerHTML = '<span>提交中...</span><i class="ri-loader-4-line joinus-btn-spin" style="display:inline-block;margin-left:8px;"></i>';
		} else {
			submitBtn.disabled = false;
			submitBtn.style.cursor = '';
			submitBtn.style.opacity = '';
			submitBtn.removeAttribute('aria-busy');
			submitBtn.classList.remove('joinus-btn-loading');
			submitBtn.innerHTML = `<span>${submitLabel}</span><i class="ri-arrow-right-line"></i>`;
		}
	}
	...
	function validateContactAndQQ(): { fieldId: string; message: string } | null {
		const contactEl = form.querySelector<HTMLInputElement>('[name="contact"]');
		const qqEl = form.querySelector<HTMLInputElement>('[name="qq"]');
		const contact = (contactEl?.value ?? '').replace(/\D/g, '');
		const qq = (qqEl?.value ?? '').trim();
		if (contact.length !== 11 || !/^1[3-9]\d{9}$/.test(contact)) {
			return { fieldId: 'contact', message: '请输入合规的 11 位手机号' };
		}
		if (!qq || !/^\d+$/.test(qq)) {
			return { fieldId: 'qq', message: 'QQ 号须为数字' };
		}
		return null;
	}

	function showErrorModal(fieldId: string, message: string, buttonLabel: '去填写' | '去修改'): void {
		const overlay = document.createElement('div');
		overlay.className = 'joinus-modal-overlay';
		overlay.setAttribute('role', 'dialog');
		overlay.setAttribute('aria-modal', 'true');
		overlay.innerHTML = `
			<div class="joinus-modal-card">
				<h2 class="joinus-modal-title">请完善信息</h2>
				<p class="joinus-modal-subtitle">${escapeHtml(message)}</p>
				<div class="joinus-modal-actions">
					<button type="button" class="joinus-btn joinus-modal-confirm">${escapeHtml(buttonLabel)}</button>
				</div>
			</div>
		`;
		const close = (): void => {
		...
		overlay.querySelector('.joinus-modal-confirm')?.addEventListener('click', () => {
			close();
			const field = form.querySelector<HTMLElement>(`.joinus-field[data-question-id="${fieldId}"]`);
	...
	function getFieldValue(field: HTMLElement, q: Question): string | number {
		const name = q.id;
		if (q.type === 'file') {
			const wrap = field.querySelector<HTMLElement>(`.joinus-file-wrap[data-name="${name}"]`);
			const w = wrap as unknown as FileWrapEl | undefined;
			return w?._files?.length ?? 0;
		}
	...
	function getFirstRequiredEmpty(): { fieldId: string; message: string } | null {
		const fields = form.querySelectorAll<HTMLElement>('.joinus-field');
		for (let i = 0; i < config.questions.length; i++) {
			const q = config.questions[i];
			const field = fields[i];
			if (!field || field.classList.contains('joinus-field-hidden')) continue;
			const required = q.required ?? false;
			if (!required) continue;
			const val = getFieldValue(field, q);
			const empty = q.type === 'file' ? (val as number) === 0 : !(val as string);
			if (empty) return { fieldId: q.id, message: `请填写「${q.label}」` };
		}
		return null;
	}
	...
	form.addEventListener('submit', async (e) => {
		e.preventDefault();
		const requiredErr = getFirstRequiredEmpty();
		if (requiredErr) {
			showErrorModal(requiredErr.fieldId, requiredErr.message, '去填写');
			return;
		}
		const err = validateContactAndQQ();
		if (err) {
			showErrorModal(err.fieldId, err.message, '去修改');
			return;
		}
		...
		for (const w of fileWraps) {
			if (!w._files?.length) {
				const wrap = form.querySelector<HTMLElement>('.joinus-file-wrap[data-required="true"]');
				const field = wrap?.closest('.joinus-field') as HTMLElement | null;
				showErrorModal(field?.dataset.questionId ?? 'portfolio', '请上传必填的附件', '去填写');
				return;
			}
		}
		...
		if (url) {
			...
			try {
				const r = await fetch(url, { method: 'POST', body: data });
				const json = (await r.json().catch(() => ({}))) as { ok?: boolean; duplicate?: boolean; error?: string };
				...
				if (!r.ok || !json.ok) {
					...
					if (elapsed < MIN_LOADING_MS) await new Promise((r) => setTimeout(r, MIN_LOADING_MS - elapsed));
					setSubmitLoading(false);
					const msg = json.error || '提交失败';
					if (/手机|联系方式/.test(msg)) showErrorModal('contact', msg, '去修改');
					else if (/QQ/.test(msg)) showErrorModal('qq', msg, '去修改');
					else showErrorModal('contact', msg, '去修改');
					return;
				}
			} catch (e) {
				const elapsed = Date.now() - startTime;
				...
				showErrorModal('contact', '提交失败: ' + (e instanceof Error ? e.message : String(e)), '去修改');
				return;
			}
			const elapsed = Date.now() - startTime;
			if (elapsed < MIN_LOADING_MS) await new Promise((r) => setTimeout(r, MIN_LOADING_MS - elapsed));
